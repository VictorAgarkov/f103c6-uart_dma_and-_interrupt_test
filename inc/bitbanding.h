...
// BIT_BAND_PERM - в качестве второго аргумента должна быть битовая маска 0x00000001 .. 0x80000000
// BIT_BAND_PERB - в качестве второго аргумента должен быть номер бита, 0 .. 31


#define BIT_BAND_PERM(REG,BIT_MASK)      (*(volatile uint32_t*)(PERIPH_BB_BASE+32*((uint32_t)(&(REG))-PERIPH_BASE)+4*((uint32_t)(MASK_TO_BIT(BIT_MASK)))))
#define BIT_BAND_PERB(REG,BIT)           (*(volatile uint32_t*)(PERIPH_BB_BASE+32*((uint32_t)(&(REG))-PERIPH_BASE)+4*((uint32_t)(BIT))))

#define BIT_BAND_SRAM(RAM,BIT)           (*(volatile uint32_t*)(SRAM_BB_BASE+32*((uint32_t)((void*)(RAM))-SRAM_BASE)+4*((uint32_t)(BIT))))

//Example1: BIT_BAND_PERM(TIM1->SR, TIM_SR_UIF) = 0; //сбросить бит TIM_SR_UIF в TIM1->SR
//Example2: BIT_BAND_PERB(TIM1->SR, 4) = 0; //сбросить бит 4 в TIM1->SR
//Example3: BIT_BAND_SRAM(&a, 13) = 1; //установить 13-й бит в переменной "a"
//Example4: BIT_BAND_SRAM(&a, 13) ^= 1; //инвертировать 13-й бит в "a", не задевая другие биты переменной (псевдо-атомарность)
#endif
